...
def importar_excel():
    ...
    try:
        ...
        # Generar número sugerido (mismo patrón que crear_manual)
        fecha_hoy = date.today().strftime('%Y%m%d')
        ultimo_hoy = Presupuesto.query.filter_by(organizacion_id=org_id).filter(
            Presupuesto.numero.like(f'PRES-{fecha_hoy}-%')
        ).order_by(desc(Presupuesto.id)).first()
        ...
        return render_template('presupuestos/importar_excel.html',
                             numero_sugerido=numero_sugerido,
                             clientes=clientes)

    except Exception as e:
        current_app.logger.error(f"Error en presupuestos.importar_excel: {e}")
        flash('Error al cargar el formulario de importación', 'danger')
        return redirect(url_for('presupuestos.lista'))


@presupuestos_bp.route('/importar-excel/preview', methods=['POST'])
@csrf.exempt
@login_required
def importar_excel_preview():
    """Parsear Excel subido y devolver preview JSON sin crear registros."""
    if not current_user.puede_gestionar():
    ...
    try:
        ...
        if filename.lower().endswith('.xls'):
            return jsonify({'error': 'Formato .xls no soportado. Por favor guardá el archivo como .xlsx desde Excel.'}), 400

        # Parse the file
        from services.excel_budget_parser import parse_excel_file
        file_stream = io.BytesIO(file.read())
        result = parse_excel_file(file_stream, filename)

        if result.file_type == 'error':
            return jsonify({'ok': False, 'error': result.warnings[0] if result.warnings else 'Error al parsear'}), 400

        # Try to match items to inventory
        for vname, items in result.items_by_vendor.items():
            for item in items:
                if item.descripcion and item.total > 0:
                    inv_match = buscar_item_inventario_por_nombre(item.descripcion, org_id)
                    if inv_match:
                        item._inv_match_id = inv_match.id
                        item._inv_match_nombre = inv_match.nombre
                    else:
                        item._inv_match_id = None
                        item._inv_match_nombre = None
                else:
                    item._inv_match_id = None
                    item._inv_match_nombre = None

        # Build response with inventory matches
        response = result.to_dict()
        for vname in response['items_by_vendor']:
            items_list = result.items_by_vendor.get(vname, [])
            item_idx = 0
            for etapa in response['items_by_vendor'][vname]['etapas']:
                for item_dict in etapa['items']:
                    if item_idx < len(items_list):
                        parsed_item = items_list[item_idx]
                        item_dict['inventario_match'] = {
                            'id': getattr(parsed_item, '_inv_match_id', None),
                            'nombre': getattr(parsed_item, '_inv_match_nombre', None),
                        }
                        from services.excel_budget_parser import _classify_tipo
                        item_dict['tipo'] = _classify_tipo(parsed_item.descripcion)
                    item_idx += 1
        ...
        current_app.logger.error(f"Error en importar_excel_preview: {e}", exc_info=True)
        return jsonify({'ok': False, 'error': f'Error al procesar: {str(e)}'}), 500


@presupuestos_bp.route('/importar-excel/confirmar', methods=['POST'])
@csrf.exempt
@login_required
def importar_excel_confirmar():
    """Crear Presupuesto + ItemPresupuesto desde datos de Excel confirmados."""
    if not current_user.puede_gestionar():
        return jsonify({'error': 'Sin permisos'}), 403
    ...
    try:
        ...
        if not numero:
            return jsonify({'error': 'El número de presupuesto es requerido'}), 400

        # Check duplicate
        existing = Presupuesto.query.filter_by(organizacion_id=org_id, numero=numero).first()
        if existing:
            return jsonify({'error': f'Ya existe un presupuesto con el número {numero}'}), 400

        cliente_id = data.get('cliente_id')
        nombre_obra = data.get('nombre_obra', '').strip()
        vigencia_dias = data.get('vigencia_dias', 30)
        vendor_name = data.get('vendor', 'Excel')
        filename_origen = data.get('filename', '')
        etapas_data = data.get('etapas', [])

        if not etapas_data:
            return jsonify({'error': 'No hay etapas/items para importar'}), 400

        # Build ia_payload for obra conversion compatibility
        ia_payload_etapas = []
        all_items_ordered = []  # Keep items in same order as payload

        for etapa in etapas_data:
            etapa_nombre = etapa.get('nombre', 'Sin Etapa')
            etapa_items = etapa.get('items', [])
            payload_items = []

            for item in etapa_items:
                cantidad = float(item.get('cantidad', 0))
                precio_unit = float(item.get('precio_unitario', 0))
                total_val = float(item.get('total', 0))
                if total_val == 0 and cantidad and precio_unit:
                    total_val = cantidad * precio_unit
                ...
                all_items_ordered.append({
                    ...
                    'item_inventario_id': item.get('item_inventario_id'),
                })

            ia_payload_etapas.append({
                'nombre': etapa_nombre,
                'items': payload_items,
            })

        ia_payload = {
            'etapas': ia_payload_etapas,
            'moneda': 'ARS',
            'origen': 'excel_import',
        }
        ...
        datos_proyecto = {
            'nombre_obra': nombre_obra,
            'modo_creacion': 'excel_import',
            'archivo_origen': filename_origen,
            'vendor': vendor_name,
        ...
        # Create ItemPresupuesto for each item (in same order as ia_payload)
        for item_data in all_items_ordered:
            ...
            if inv_id:
                ...
                # Try auto-matching
                matched = buscar_item_inventario_por_nombre(item_data['descripcion'], org_id)
                inv_id = matched.id if matched else None

            item_pres = ItemPresupuesto(
                presupuesto_id=presupuesto.id,
                tipo=item_data['tipo'],
                ...
                origen='importado',
                currency='ARS',
                price_unit_ars=precio_unit,
                total_ars=subtotal,
                item_inventario_id=inv_id,
            )
            db.session.add(item_pres)
        ...
        current_app.logger.info(
            f"Presupuesto {numero} importado desde Excel ({vendor_name}) "
            f"con {len(all_items_ordered)} items"
        )

        return jsonify({
            'ok': True,
            'presupuesto_id': presupuesto.id,
            'redirect_url': url_for('presupuestos.detalle', id=presupuesto.id),
        }), 200

    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Error en importar_excel_confirmar: {e}", exc_info=True)
        return jsonify({'ok': False, 'error': f'Error al importar: {str(e)}'}), 500
